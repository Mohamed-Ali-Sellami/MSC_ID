...
export const userRegister = createAsyncThunk("user/register", async (user, { rejectWithValue }) => {
  try {
    const response = await axios.post("https://msc-id.onrender.com/user/register", user);
...
export const userLogin = createAsyncThunk("user/login", async (user, { rejectWithValue }) => {
  try {
    const response = await axios.post("https://msc-id.onrender.com/user/login", user);
    return response.data;
  } catch (error) {
    console.error(error);
...
export const userCurrent = createAsyncThunk("user/current", async (_, { rejectWithValue }) => {
  try {
    const response = await axios.get("https://msc-id.onrender.com/user/current", {
      headers: {
...
export const updateuser = createAsyncThunk("user/update", async ({ id, updatedData }, { rejectWithValue }) => {
  try {
    const response = await axios.put(`https://msc-id.onrender.com/user/${id}`, updatedData);
    return response.data;
  } catch (error) {
    console.error(error);
...
export const deleteuser = createAsyncThunk("user/delete", async (id, { rejectWithValue }) => {
  try {
    await axios.delete(`https://msc-id.onrender.com/user/${id}`);
